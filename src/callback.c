...
// 新增的once相关变量
int onceEnabled = 0;       // 是否启用once模式
FILE *onceFile = NULL;     // once模式下的文件指针
...
int enable_once_output(const char *fileName)
{
    ...
    // 打开文件
    onceFile = fopen(fileName, "w");
    if (onceFile == NULL)
    {
        perror("Error opening once output file");
        return -1;
    }
    
    onceEnabled = 1;
    printf("Next SQL statement will output to file: %s\n", fileName);
    return 0;
}
...
int callback(void *data, int argc, char **azColName, char **argv)
{
    ...
    if (ifExcel)
    {
        if (headerPrinted == 0)
        {
            ...
            // once模式下也写入文件
            if (onceEnabled && onceFile != NULL)
            {
                for (int i = 0; i < argc; i++)
                {
                    fprintf(onceFile, "\"%s\"", azColName[i]);
                    if (i < argc - 1)
                    {
                        fprintf(onceFile, "%s", colSeparator);
                    }
                }
                fprintf(onceFile, "%s", rowSeparator);
            }
        }
        ...
        if (onceEnabled && onceFile != NULL)
        {
            for (int i = 0; i < argc; i++)
            {
                fprintf(onceFile, "\"%s\"", argv[i] ? argv[i] : "NULL");
                if (i < argc - 1)
                {
                    fprintf(onceFile, "%s", colSeparator);
                }
        ...
        return 0; // Excel模式直接返回，避免继续执行普通输出
    }
    else
    {
        // 普通输出（不为电子表格格式）
        if (printColumnNames && !headerPrinted)
        {
            for (int i = 0; i < argc; i++)
            {
                ...
                if (onceEnabled && onceFile != NULL)
                {
                    fprintf(onceFile, "%-*s%s", columnWidth, azColName[i], colSeparator);
                }
            }
            printf("%s", rowSeparator);
            if (onceEnabled && onceFile != NULL)
            ...
            for (int i = 0; i < argc; i++)
            {
                ...
                if (onceEnabled && onceFile != NULL)
                {
                    fprintf(onceFile, "%-*s%s", columnWidth, "------", colSeparator);
                }
            }
            printf("%s", rowSeparator);
            if (onceEnabled && onceFile != NULL)
        ...
        for (int i = 0; i < argc; i++)
        {
            ...
            if (onceEnabled && onceFile != NULL)
            {
                fprintf(onceFile, "%-*s%s", columnWidth, argv[i] ? argv[i] : "NULL", colSeparator);
            }
        }
        printf("%s", rowSeparator);
        if (onceEnabled && onceFile != NULL)
